##Two-stage model with two-species fertility competition
...
pdf(file="2Spe-2LF_iteroparous_gamma1.pdf",width = 8, height = 8)
par(mfrow=c(2,2))
for(i in 1:3){
  plot(TimeVec,N1j[i,]+N1a[i,],type="l",
       xlab='Time (years)',ylab='Population Density', cex.main = 1.2,
       ylim=c(0,1.2*max(N1j[i,], N2j[i,], N1a[i,], N2a[i,])),cex.lab = 1.2,lty = 1, lwd = 1 , col='cyan')
  lines(TimeVec,N2j[i,]+N2a[i,],lty = 1,lwd=2,col='orange')
}
dev.off()

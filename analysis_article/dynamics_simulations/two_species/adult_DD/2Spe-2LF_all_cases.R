##III - Two-stage model with two-species fertility competition
### Calculus for all parameter sets of table 1
### Different cases of life-cycle structure (semelparous/iteroparous and fast maturation/delayed maturation)

#1. Clearing workspace
rm(list=ls())
graphics.off()

#2. Parameters set on types (lambda 1, lambda 2, alphas...)

fert1<-c(30,30,30)
fert2<-c(25,25,25)

matur1<-c(0.7,0.7,0.7)
matur2 <-c(0.8,0.8,0.8)

#juvenile survival
phi1 <-c(0.5,0.5,0.5)
phi2 <-c(0.4,0.4,0.4)

#adult survival
s1a<-c(0.5,0.5,0.5)
s2a<-c(0.6,0.6,0.6)


Alphaset<-list(matrix(c(0.1, 0.05, 0.06, 0.1),ncol = 2, byrow = TRUE),
               matrix(c(0.1, 0.02, 0.112, 0.1),ncol = 2, byrow = TRUE),
               matrix(c(0.1, 0.043, 0.035, 0.1),ncol = 2, byrow = TRUE))

Betaset<-list(matrix(c(0.1, 0.06, 0.06, 0.1),ncol = 2, byrow = TRUE),
              matrix(c(0.1, 0.125, 0.01, 0.1),ncol = 2, byrow = TRUE),
              matrix(c(0.1, 0.155, 0.165, 0.1),ncol = 2, byrow = TRUE))


#3. Creating vectors
years <- 300
TimeVec<-seq(1,years+1)
N1j<-matrix(0,3,years+1)
N2j<-matrix(0,3,years+1)
N1a<-matrix(0,3,years+1)
N2a<-matrix(0,3,years+1)
svar1j<-matrix(0,3,years+1)
svar2j<-matrix(0,3,years+1)

#4. INITIALIZATION
N0 <- 50
for(i in 1:3){
  N1j[i,1] <-0
  N1a[i,1] <-N0
  N2j[i,1] <-0
  N2a[i,1] <-N0
}

for (t in 1:years){
  for(i in 1:3){ 
    alphs <- Alphaset[[i]]
    betas <- Betaset[[i]]
    
    svar1j[i,t] <- phi1[i]/(1+betas[1,1]*N1a[i,t]+betas[1,2]*N2a[i,t])
    svar2j[i,t] <- phi2[i]/(1+betas[2,1]*N1a[i,t]+betas[2,2]*N2a[i,t]) ### corrected indices betas[2,1] and betas[2,2] 29/04/2020
    
    N1j[i,t+1]<-(1-matur1[i])*svar1j[i,t]*N1j[i,t]+
      (fert1[i]/(1+alphs[1,1]*N1a[i,t]+alphs[1,2]*N2a[i,t])*N1a[i,t]) ### corrected svar1j[i] -> svar1j[i,t] 29/04/2020
    N2j[i,t+1]<-(1-matur2[i])*svar2j[i,t]*N2j[i,t]+
      (fert2[i]/(1+alphs[2,2]*N2a[i,t]+alphs[2,1]*N1a[i,t])*N2a[i,t]) ### corrected svar2j[i]-> svar2j[i,t] 29/04/2020
    
    N1a[i,t+1]<-matur1[i]*svar1j[i,t]*N1j[i,t]+s1a[i]*N1a[i,t]  ### corrected svar1j[i] -> svar1j[i,t] 29/04/2020
    N2a[i,t+1]<-matur2[i]*svar2j[i,t]*N2j[i,t]+s2a[i]*N2a[i,t]  ### corrected svar2j[i]-> svar2j[i,t] 29/04/2020
  }
}


pdf(file="2Spe-2LF_all_cases.pdf",width = 8, height = 8)
par(mfrow=c(4,3))
main_titles = c('Parameter set 1', 'Parameter set 2', 'Parameter set 3')
ylabs = c('Population Density', '', '')
for(i in 1:3){
  plot(TimeVec,N1j[i,]+N1a[i,],type="l",
       xlab='',ylab=ylabs[i], cex.main = 1.2,
       ylim=c(0,1.2*max(N1j[i,], N2j[i,], N1a[i,], N2a[i,])),cex.lab = 1.2,lty = 1, lwd = 1 , col='cyan')
  lines(TimeVec,N2j[i,]+N2a[i,],lty = 1,lwd=1,col='orange')
  title(main='Iteroparous - delayed maturation', line=0.5)
  mtext(text = main_titles[i], line = 2, side=3)
}

#### iteroparous & gamma=1

#juvenile survival
matur1<-c(1,1,1)
matur2 <-c(1,1,1)

#adult survival

s1a<-c(0.5,0.5,0.5)
s2a<-c(0.6,0.6,0.6)


years <- 300
TimeVec<-seq(1,years+1)
N1j<-matrix(0,3,years+1)
N2j<-matrix(0,3,years+1)
N1a<-matrix(0,3,years+1)
N2a<-matrix(0,3,years+1)
svar1j<-matrix(0,3,years+1)
svar2j<-matrix(0,3,years+1)

N0 <- 50
for(i in 1:3){
  N1j[i,1] <-0
  N1a[i,1] <-N0
  N2j[i,1] <-0
  N2a[i,1] <-N0
}

for (t in 1:years){
  for(i in 1:3){ 
    alphs <- Alphaset[[i]]
    betas <- Betaset[[i]]
    
    svar1j[i,t] <- phi1[i]/(1+betas[1,1]*N1a[i,t]+betas[1,2]*N2a[i,t])
    svar2j[i,t] <- phi2[i]/(1+betas[2,1]*N1a[i,t]+betas[2,2]*N2a[i,t]) ### corrected indices betas[2,1] and betas[2,2] 29/04/2020
    
    N1j[i,t+1]<-(1-matur1[i])*svar1j[i,t]*N1j[i,t]+
      (fert1[i]/(1+alphs[1,1]*N1a[i,t]+alphs[1,2]*N2a[i,t])*N1a[i,t]) ### corrected svar1j[i] -> svar1j[i,t] 29/04/2020
    N2j[i,t+1]<-(1-matur2[i])*svar2j[i,t]*N2j[i,t]+
      (fert2[i]/(1+alphs[2,2]*N2a[i,t]+alphs[2,1]*N1a[i,t])*N2a[i,t]) ### corrected svar2j[i]-> svar2j[i,t] 29/04/2020
    
    N1a[i,t+1]<-matur1[i]*svar1j[i,t]*N1j[i,t]+s1a[i]*N1a[i,t]  ### corrected svar1j[i] -> svar1j[i,t] 29/04/2020
    N2a[i,t+1]<-matur2[i]*svar2j[i,t]*N2j[i,t]+s2a[i]*N2a[i,t]  ### corrected svar2j[i]-> svar2j[i,t] 29/04/2020
  }
}

ylabs = c('Population Density', '', '')
for(i in 1:3){
  plot(TimeVec,N1j[i,]+N1a[i,],type="l",
       xlab='',ylab=ylabs[i], cex.main = 1.2,
       ylim=c(0,1.2*max(N1j[i,], N2j[i,], N1a[i,], N2a[i,])),cex.lab = 1.2,lty = 1, lwd = 1 , col='cyan')
  lines(TimeVec,N2j[i,]+N2a[i,],lty = 1,lwd=1,col='orange')
  title(main='Iteroparous - fast maturation', line=0.5)
}


#### semelparous & gamma < 1


#adult survival

s1a<-c(0,0,0)
s2a<-c(0,0,0)

matur1<-c(0.7,0.7,0.7)
matur2 <-c(0.8,0.8,0.8)

years <- 300
TimeVec<-seq(1,years+1)
N1j<-matrix(0,3,years+1)
N2j<-matrix(0,3,years+1)
N1a<-matrix(0,3,years+1)
N2a<-matrix(0,3,years+1)
svar1j<-matrix(0,3,years+1)
svar2j<-matrix(0,3,years+1)

N0 <- 50
for(i in 1:3){
  N1j[i,1] <-0
  N1a[i,1] <-N0
  N2j[i,1] <-0
  N2a[i,1] <-N0
}

for (t in 1:years){
  for(i in 1:3){ 
    alphs <- Alphaset[[i]]
    betas <- Betaset[[i]]
    
    svar1j[i,t] <- phi1[i]/(1+betas[1,1]*N1a[i,t]+betas[1,2]*N2a[i,t])
    svar2j[i,t] <- phi2[i]/(1+betas[2,1]*N1a[i,t]+betas[2,2]*N2a[i,t]) ### corrected indices betas[2,1] and betas[2,2] 29/04/2020
    
    N1j[i,t+1]<-(1-matur1[i])*svar1j[i,t]*N1j[i,t]+
      (fert1[i]/(1+alphs[1,1]*N1a[i,t]+alphs[1,2]*N2a[i,t])*N1a[i,t]) ### corrected svar1j[i] -> svar1j[i,t] 29/04/2020
    N2j[i,t+1]<-(1-matur2[i])*svar2j[i,t]*N2j[i,t]+
      (fert2[i]/(1+alphs[2,2]*N2a[i,t]+alphs[2,1]*N1a[i,t])*N2a[i,t]) ### corrected svar2j[i]-> svar2j[i,t] 29/04/2020
    
    N1a[i,t+1]<-matur1[i]*svar1j[i,t]*N1j[i,t]+s1a[i]*N1a[i,t]  ### corrected svar1j[i] -> svar1j[i,t] 29/04/2020
    N2a[i,t+1]<-matur2[i]*svar2j[i,t]*N2j[i,t]+s2a[i]*N2a[i,t]  ### corrected svar2j[i]-> svar2j[i,t] 29/04/2020
  }
}

ylabs = c('Population Density', '', '')
for(i in 1:3){
  plot(TimeVec,N1j[i,]+N1a[i,],type="l",
       xlab='',ylab=ylabs[i], cex.main = 1.2,
       ylim=c(0,1.2*max(N1j[i,], N2j[i,], N1a[i,], N2a[i,])),cex.lab = 1.2,lty = 1, lwd = 1 , col='cyan')
  lines(TimeVec,N2j[i,]+N2a[i,],lty = 1,lwd=1,col='orange')
  title(main='Semeparous - delayed maturation', line=0.5)
}



#### semelparous & gamma = 1


#adult survival

s1a<-c(0,0,0)
s2a<-c(0,0,0)

matur1<-c(1,1,1)
matur2 <-c(1,1,1)

years <- 300
TimeVec<-seq(1,years+1)
N1j<-matrix(0,3,years+1)
N2j<-matrix(0,3,years+1)
N1a<-matrix(0,3,years+1)
N2a<-matrix(0,3,years+1)
svar1j<-matrix(0,3,years+1)
svar2j<-matrix(0,3,years+1)

N0 <- 50
for(i in 1:3){
  N1j[i,1] <-0
  N1a[i,1] <-N0
  N2j[i,1] <-0
  N2a[i,1] <-N0
}

for (t in 1:years){
  for(i in 1:3){ 
    alphs <- Alphaset[[i]]
    betas <- Betaset[[i]]
    
    svar1j[i,t] <- phi1[i]/(1+betas[1,1]*N1a[i,t]+betas[1,2]*N2a[i,t])
    svar2j[i,t] <- phi2[i]/(1+betas[2,1]*N1a[i,t]+betas[2,2]*N2a[i,t]) ### corrected indices betas[2,1] and betas[2,2] 29/04/2020
    
    N1j[i,t+1]<-(1-matur1[i])*svar1j[i,t]*N1j[i,t]+
      (fert1[i]/(1+alphs[1,1]*N1a[i,t]+alphs[1,2]*N2a[i,t])*N1a[i,t]) ### corrected svar1j[i] -> svar1j[i,t] 29/04/2020
    N2j[i,t+1]<-(1-matur2[i])*svar2j[i,t]*N2j[i,t]+
      (fert2[i]/(1+alphs[2,2]*N2a[i,t]+alphs[2,1]*N1a[i,t])*N2a[i,t]) ### corrected svar2j[i]-> svar2j[i,t] 29/04/2020
    
    N1a[i,t+1]<-matur1[i]*svar1j[i,t]*N1j[i,t]+s1a[i]*N1a[i,t]  ### corrected svar1j[i] -> svar1j[i,t] 29/04/2020
    N2a[i,t+1]<-matur2[i]*svar2j[i,t]*N2j[i,t]+s2a[i]*N2a[i,t]  ### corrected svar2j[i]-> svar2j[i,t] 29/04/2020
  }
}

ylabs = c('Population Density', '', '')
for(i in 1:3){
  plot(TimeVec,N1j[i,]+N1a[i,],type="l",
       xlab='Time (years)',ylab=ylabs[i], cex.main = 1.2,
       ylim=c(0,1.2*max(N1j[i,], N2j[i,], N1a[i,], N2a[i,])),cex.lab = 1.2,lty = 1, lwd = 1 , col='cyan')
  lines(TimeVec,N2j[i,]+N2a[i,],lty = 1,lwd=1,col='orange')
  title(main='Semeparous - fast maturation', line=0.5)
  
}




dev.off()

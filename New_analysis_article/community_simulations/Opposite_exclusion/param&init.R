...
parametrage <- function (S){
  fert = rlnorm(S, meanlog = 3, sdlog=0.005)  ## log-normal distribution for fertility, low variance to avoid too large fertility
  mat_rate = rbeta(S, 500,500) ## beta distribution centered in 1/2
  juv_surv = rbeta(S, 500,500) ## beta distribution centered in 1/2
  adult_surv = rbeta(S, 500,500) ## beta distribution centered in 1/2
  ...
  for (i in 1:S){
    for (j in 1:i){
      if (i==j){
        ...
        x1 <- alpha_beta_inter[c,] ## alpha and beta competition coefficient
        x2 <- alpha_beta_inter[c+1,]
        alpha[i,j] <- max(x1) ## max and min to have the negative correlation
        beta[i,j] <- min(x1) 
        alpha[j,i] <- min(x2) ## inverted max and min to promote reciprocal exclusion
        beta[j,i] <- max(x2)
        c=c+2
      }
    }

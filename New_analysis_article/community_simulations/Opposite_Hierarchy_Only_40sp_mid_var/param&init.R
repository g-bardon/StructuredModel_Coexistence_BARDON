...
parametrage <- function (S){
  fert = rlnorm(S, meanlog = 3, sdlog=0.05)   ## log-normal distribution for fertility, low variance to avoid too large fertility
  mat_rate = rbeta(S, 50,50) ## beta distribution centered in 1/2
  juv_surv = rbeta(S, 50,50) ## beta distribution centered in 1/2
  adult_surv = rbeta(S, 50,50) ## beta distribution centered in 1/2

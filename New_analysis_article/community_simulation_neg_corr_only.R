...
parametrage <- function (S){
  ...
  sig=0.03
  phi=0.95
  alpha_beta_inter = rmvnorm(n=S*(S-1), mean=c(0.04,0.04), sigma=matrix(c(sig^2, -(sig^2)*phi, -(sig^2)*phi, sig^2), 2,2, byrow = TRUE)) 
...
### Tirage d'un jeu de param -> check et stockage du jeu -> ensuite permutation et conclusion sur le jeu de param
### Voir comment ont été faites les simul de Moll et Brown et Fujiwara
### Voir si nos conclusions sur 2 esp sont valables si ont modif bcp les parametres
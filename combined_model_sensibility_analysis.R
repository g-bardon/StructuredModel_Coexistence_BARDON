### Application of Barabas method (Barabas et al. 2012) to the combined competition model with 2 species and 2 life stages
### Gael Bardon
### 29/04/2020
...
######## Simulation for estimation of fixed point ########
# Stop criteria as in barabas et al. 2012
...
lambda_test1=0
lambda_test2=0
while (abs(lambda_test1-1)>10^-5 && abs(lambda_test2-1)>10^-5){
...
# Calculus of regulating variables, eigenvalues, eigenvectors 
n1j= n1[1]
n1a=n1[2]
n2j= n2[1]
n2a=n2[2]
...
prod_kro1 = 1/(lambda1[1]-lambda1[2])*kronecker(matrix(c(w1[,2]-q%*%w1[,2]%*%w1[,1]), 2,1),matrix(v1[,2], 1,2))    ## kronecker product to simplify calculus
...
### Calculus of sensibilities (see eq 31)
Mij_inv = solve(Mij)
...
param <- t(as.matrix(data.frame(pi01, pi02,gamma1, gamma2,phi1,phi2, s1a,s2a,alpha11,alpha12,alpha21,alpha22,beta11,beta12,beta21,beta22)))
result <- cbind(param,dn1_dE,dn2_dE)
